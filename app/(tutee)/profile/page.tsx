...
export default function TuteeProfile() {
  ...
  useEffect(() => {
    ...
    setAvailability(normalizeAvailability(currentPublicMetadata?.tuteeAvailability));
  }, [user]);

  const fullName = useMemo(() => {
    const merged = `${user?.firstName || ""} ${user?.lastName || ""}`.trim();
    return merged || "Unnamed Student";
  ...
  const onSave = async () => {
    ...
    try {
      const result = await updateProfilePublicMetadata({
        availability,
        bio: form.bio,
        availabilityKey: "tuteeAvailability",
      });
    ...
    <div className="container mx-auto py-6 px-4 space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage your account details without leaving the dashboard.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="secondary">Tutee</Badge>
