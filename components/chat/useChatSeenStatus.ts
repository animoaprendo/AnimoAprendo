...
export function useChatSeenStatus(userId: string, isWindowFocused: boolean, allChats: Message[]) {
  ...
  const recalculateUnreadCounts = useCallback(() => {
    return (prevUsers: User[]) => 
      prevUsers.map(user => {
        ...
        const messagesFromOtherUser = allChats.filter((msg: Message) => 
        ...
        const unreadMessages = messagesFromOtherUser.filter((msg: Message) => 
          !msg.seenBy || !msg.seenBy.includes(userId) // Message not seen by current user
        );
  ...
  const markMessagesAsSeen = useCallback(async (conversationUserId: string) => {
