...
export function useMessageManagement(userId: string, userRole: "tutee" | "tutor") {
  // Client-side message deduplication
  const [pendingMessages, setPendingMessages] = useState<Set<string>>(new Set());
  const [sentMessageIds, setSentMessageIds] = useState<Set<string>>(new Set());

  // Generate unique message ID for client-side deduplication
  const generateMessageId = useCallback((content: string, timestamp: number, activeUserId?: string): string => {
  ...
  const handleSendMessage = useCallback(async (
    ...
    const messageId = generateMessageId(newMessage, Date.now(), activeUser.id);
    ...
    if (pendingMessages.has(messageId) || sentMessageIds.has(messageId)) {
      console.log("Message already being sent or recently sent, ignoring duplicate");
      return;
    }
    ...
    if (recentDuplicateId) {
      console.log("Duplicate message content detected in last 5 seconds, ignoring");
      return;
    }
